...
import api_v1.containers.loop.membership.add_loop_members.serializers as serializers
...
@api_view(['POST'])
def add_loop_members(request, format=None):
  data = copy.deepcopy(request.data)

  serializer = serializers.AddLoopMembersSerializer(data=request.data)
  ...
  for invitee in invitees:
    try:
      ...
      invitation = Invitation.create(
        invitation_type = Invitation.BECOME_GROUP_MEMBER,
        group = group,
        inviter = user,
        invitee = invitee,
