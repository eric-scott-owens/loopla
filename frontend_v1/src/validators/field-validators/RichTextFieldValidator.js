...
export default class RichTextFieldValidator extends BaseFieldValidator {
  ...
  async validate(value) {
    ...
    const text = value ? Plain.serialize(value) : value;
    const json = value ? JSON.stringify(value.toJSON()) : value;
    ...
    if(this.maxLength > 0) {
      if(json && json.length > this.maxLength) {
    ...
    if(this.isAlphaNumeric === true) {
      if(text && !validator.isAlphanumeric(text)) {
        validationErrorMessages.push(`${this.interjections.isAlphaNumeric}! Use letters and numbers only.`);
      }
    }
